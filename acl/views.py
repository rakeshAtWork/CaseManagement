...
from .serializers import (RoleMultiUserCreateSerializer, RoleFilterSerializer, RoleReadSerializer,
                          ...
                          PermissionSerializer, AppConfigurationSerializer, MasterModuleSerializer, )
from django.core.paginator import Paginator
from .models import Role, UserRole, MasterPrivilege, RolePermission, AppConfiguration, MasterModule
...
class RoleFilterApi(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.warning(f'Error at RoleFilterList: {str(ve)}')
            raise serializers.ValidationError(ve.detail)
        except Exception as ee:
            logger.warning(f'Error at RoleFilterList: {str(ee)}')
...
class RoleCreateApi(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        after_instance = Role.objects.get(pk=instance.pk)
        log_activity(
            instance=instance,
...
class RoleUpdateApi(RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        before_instance = Role.objects.get(pk=instance.pk)
...
class PermissionFilterApi(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.warning(f"Unexpected error at Permission Filter: {str(ee)}")
...
class AppConfigurationListCreateAPIView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        after_instance = AppConfiguration.objects.get(pk=instance.pk)
        log_activity(
            instance=instance,
            request=self.request,
            action_type='CREATE',
            before_instance=None,
            after_instance=after_instance,
            description="App Configuration Created",
        )
...
class AppConfigurationRetrieveUpdateDestroyAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        instance = self.get_object()
        before_instance = AppConfiguration.objects.get(pk=instance.pk)
        ...
        log_activity(
            ...
            before_instance=before_instance,
            after_instance=after_instance,
            description="App Configurations Updated",
        )

    def perform_destroy(self, instance):
        before_instance = AppConfiguration.objects.get(pk=instance.pk)
        # Set deleted_at and save the instance
        ...
        log_activity(
            ...
            before_instance=before_instance,
            after_instance=None,
            description="App Configuration Deleted",
        )


class MasterModuleListCreateView(ListCreateAPIView):
    queryset = MasterModule.objects.all()
    serializer_class = MasterModuleSerializer
    pagination_class = None


class PopulateMasterModuleView(APIView):
    def post(self, request):
        populated_modules = []
        for module in ModuleEnum:
            ...
            # Create or update the MasterModule instance
            obj, created = MasterModule.objects.update_or_create(
                module_id=module_id,
                defaults={'name': module_name}
            )
            populated_modules.append(obj)

        return Response(
            {
                "message": "MasterModule populated successfully.",
                "data": [f"{module.module_id}: {module.name}" for module in populated_modules]
            },
            status=status.HTTP_201_CREATED
        )


class DeleteAllPrivilegesApiView(APIView):
    """
    API endpoint to delete all privileges.
    """

    def delete(self, request, *args, **kwargs):
        # Delete all privileges
        MasterPrivilege.objects.all().delete()
        return Response({"message": "All privileges have been deleted."}, status=status.HTTP_204_NO_CONTENT)
