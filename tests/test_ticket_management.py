...
from ticket_management.models import SLA, Department
import uuid
...
#         url = reverse('ticket_type_create')
#         data = {
#             "name": "Bug",
#             "is_active": True
#         }
...
#         )
#         self.update_url = reverse('ticket_type_update', kwargs={'pk': self.ticket_type.id})
#
...
#         url = reverse('ticket_type_update', kwargs={'pk': non_existent_id})
#         data = {
#             "name": "Feature",
#             "is_active": False
#         }
...
class SLARetrieveUpdateDeleteTest(BaseTestCase):
    def setUp(self):
        ...
        self.sla = SLA.objects.create(
            ...
            resolution_time="02:00:00",  # Use datetime.time correctly
            created_by=self.user.id
        )
    ...
    def test_update_sla_success(self):
        data = {
            "department": self.department.id,
            "response_time": "02:00:00",
            "resolution_time": "04:00:00"
        }
        response = self.client.put(self.retrieve_url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.sla.refresh_from_db()
        self.assertEqual(self.sla.response_time.strftime('%H:%M:%S'), "02:00:00")
        self.assertEqual(self.sla.resolution_time.strftime('%H:%M:%S'), "04:00:00")

    def test_delete_sla_success(self):
        response = self.client.delete(self.retrieve_url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.sla.refresh_from_db()
        self.assertTrue(self.sla.is_delete)  # Check if the SLA is marked as deleted
    ...
    #     self.client.credentials()  # Remove authentication
    #     data = {
    #         "response_time": "02:00:00",  # Ensure valid data is sent
    #         "resolution_time": "04:00:00"
    #     }
    #     response = self.client.put(self.retrieve_url, data, format='json')
...
class SLAFilterApiTest(BaseTestCase):
    def setUp(self):
        ...
        # Use string representation of timedelta
        self.sla1 = SLA.objects.create(
            department=self.department1,
            response_time="01:00:00",  # Use ISO 8601 format string
            resolution_time="02:00:00",  # Use ISO 8601 format string
            is_active=True,
            created_by=self.user.id
        )
        self.sla2 = SLA.objects.create(
            department=self.department2,
            response_time="02:00:00",  # Use ISO 8601 format string
            resolution_time="04:00:00",  # Use ISO 8601 format string
            is_active=False,
            created_by=self.user.id
        )
    ...
    def test_filter_sla_by_response_time_range(self):
        ...
        data = {
            "response_time_min": "01:00:00",  # Use ISO 8601 format string
            "response_time_max": "02:00:00"   # Use ISO 8601 format string
        }
        response = self.client.post(self.filter_url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], 2)
